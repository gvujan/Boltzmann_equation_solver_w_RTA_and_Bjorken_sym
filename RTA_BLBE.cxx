...
int const four_pi_eta_bar=10; // Note that eta_bar is specific shear viscosity (i.e. shear viscosity over entropy density or eta/s).
...
const int N_tau=1001;                                     // Number of tau steps calculated.
double const tau_min=0.25;                                // Minimum proper time tau.      
double const tau_max=10.25;                               // Maximum proper time tau.
...
int main(void)
{
  ...
  fp_beta=fopen("beta_vs_tau.dat", "w");
  double const beta_0=0.197/0.3; // Inverse temperature at tau_min. 
  double beta[N_tau];             
  for(itau=0;itau<N_tau;itau++){

      // First guess for the temeprature profile to be iterated over.
      beta[itau]=beta_0*pow(tau[itau]/tau_min,1.0/3.0); 
      fprintf(fp_beta, "%d %.10e %.10e\n", 0, tau[itau], beta[itau]); // Saving the first guess to disk.
